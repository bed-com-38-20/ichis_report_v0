...
const DroppableTable = ({ config, orgUnits, periods, onConfigChange }) => {
  const { columns = ['Period', 'Organisation Unit'], data = {}, calculatedFields = [] } = config;
  ...
  const removeColumn = (column) => {
    ...
    onConfigChange({ ...config, columns: newColumns, data: newData });
  };

  const getCellValue = (period, orgUnit, column) => {
    const key = `${period.id}-${orgUnit.id}-${column}`;
    const field = calculatedFields.find(f => f.name === column);
    if (field) {
      return evaluateFormula(field.formula, period, orgUnit, data);
    }
  ...
  return (
    <div ref={drop} style={styles.dropArea(isOver)}>
      <DataTable>
        <DataTableRow>
          {columns.map((column, index) => (
            <ColumnHeader key={column} column={column} index={index} moveColumn={moveColumn}>
              <div style={styles.columnContent}>
                {column}
                {!['Period', 'Organisation Unit'].includes(column) && (
                  <Button small destructive onClick={(e) => {
                    e.stopPropagation();
                    removeColumn(column);
        ...
        {periods.flatMap(period => orgUnits.map(orgUnit => (
          <DataTableRow key={`${period.id}-${orgUnit.id}`}>
            {columns.map(column => {
              if (column === 'Period') return <DataTableCell key="period">{period.displayName}</DataTableCell>;
              if (column === 'Organisation Unit') return <DataTableCell key="orgUnit">{orgUnit.displayName}</DataTableCell>;
              return (
                <TableCellDropTarget
                  key={column}
                  column={column}
                  onDrop={(item) => handleCellDrop(item, period, orgUnit, column)}
                >
                  {getCellValue(period, orgUnit, column)}
                </TableCellDropTarget>
              );
            })}
          </DataTableRow>
