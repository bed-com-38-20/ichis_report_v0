...
import { Button, Card, CircularLoader, InputField, SingleSelect, SingleSelectOption, Menu, MenuItem, Tooltip, Divider } from '@dhis2/ui';
...
const DraggableItem = ({ item, onRemove }) => {
  ...
  return (
    ...
    >
      <Tooltip 
        content={`${item.type === 'indicator' ? 'Indicator' : 'Data Element'}: ${item.metadata?.indicatorType?.displayName || item.metadata?.categoryCombo?.displayName}`}
      >
